...
export function getMovies() {
  return function (dispatch) {
    return axios
      .get("http://api-pf-cine.herokuapp.com/movies")
      .then((r) => r.data)
...
export function getMovieDetail(id) {
  return function (dispatch) {
    return axios
      .get(`http://api-pf-cine.herokuapp.com/movies/${id}`)
      .then((r) => r.data)
...
export function getMovieName(name) {
  return async function (dispatch) {
    const json = await axios.get(
      `http://api-pf-cine.herokuapp.com/movies?name=${name}`
    );
    return dispatch({
      type: "GET_MOVIE_NAME",
      payload: json.data,
...
export const deleteMovie = (id) => async (dispatch) => {
  try {
    const response = await axios.delete(
      `http://api-pf-cine.herokuapp.com/movies/delete/${id}`
    );
    dispatch(getMovies());
  } catch (error) {
    console.log(error);
